...
/// Accepting a function completion leaves the cursor inside the parentheses,
/// not after the closer. The kit has no snippet support, so the app places the
/// cursor itself after the accept's `InputEvent::Change`.
// The env guard serializes this process-global config dir for the whole test;
// holding it across the `wait_for` await is intentional, not a deadlock risk.
#[allow(clippy::await_holding_lock)]
#[gpui_kit::test]
async fn function_completion_puts_cursor_inside_parens(cx: &mut TestAppContext) {
    let _guard = env_guard();
    cx.update(gpui_kit::init);
    let dir = staged_dir("fn-parens");
    std::env::set_var("SQLHIGHLAND_CONFIG_DIR", &dir);
    let slot: std::rc::Rc<std::cell::RefCell<Option<gpui_kit::Entity<SqlHighlandView>>>> =
        std::rc::Rc::new(std::cell::RefCell::new(None));
    let for_window = slot.clone();
    ...
    // Type a real prefix; the provider runs from the keystroke (not a re-run),
    // exactly as in the app.
    cx.update_window(handle.into(), |_, window, cx| {
        window.render_frame(cx);
        window.input("SELECT LOW", cx);
        *editor_out.borrow_mut() = Some(view.read(cx).debug_active_editor());
    })
    .unwrap();

    // The completion popup is populated by an async task.
    let editor = editor_slot.borrow().clone().expect("editor captured");
    cx.wait_for(handle.into(), std::time::Duration::from_secs(2), |_, cx| {
        editor.read(cx).completion_menu_state().open
    })
    .await;

    // Accept the already-presented item through the real insert path.
    cx.update_window(handle.into(), |_, window, cx| {
        view.update(cx, |this, cx| {
            this.debug_accept_completion("LOWER()", window, cx);
        });
    })
    .unwrap();

    // The cursor move is deferred (the editor is leased during the change).
    cx.run_until_parked();

    cx.update_window(handle.into(), |_, _, cx| {
        let value = editor.read(cx).value().to_string();
        assert_eq!(value, "SELECT LOWER()", "completion inserted the call form");
        assert_eq!(
            editor.read(cx).cursor(),
            value.len() - 1,
            "cursor should sit between the parentheses"
        );
    })
    .unwrap();
    let _ = std::fs::remove_dir_all(&dir);
...
/// Typing a function call by hand must not have its cursor relocated: with the
/// popup open, typing `lower` then `(` auto-closes to `lower(|)` and leaves the
/// cursor inside already, so the accept-placement path must leave it alone.
#[gpui_kit::test]
async fn hand_typed_call_cursor_is_not_moved(cx: &mut TestAppContext) {
    let _guard = env_guard();
    cx.update(gpui_kit::init);
    let dir = staged_dir("fn-hand");
    std::env::set_var("SQLHIGHLAND_CONFIG_DIR", &dir);
    let slot: std::rc::Rc<std::cell::RefCell<Option<gpui_kit::Entity<SqlHighlandView>>>> =
        std::rc::Rc::new(std::cell::RefCell::new(None));
    let for_window = slot.clone();
    ...
    cx.update_window(handle.into(), |_, window, cx| {
        window.render_frame(cx);
        // The popup is open from "low", then `(` is typed by hand: auto-close
        // inserts the closer and puts the cursor between the parens.
        window.input("SELECT lower", cx);
        window.input("(", cx);
        *editor_out.borrow_mut() = Some(view.read(cx).debug_active_editor());
    })
    .unwrap();
    ...
    cx.update_window(handle.into(), |_, _, cx| {
        let value = editor.read(cx).value().to_string();
        assert_eq!(value, "SELECT lower()", "auto-close inserted the pair");
        assert_eq!(
            editor.read(cx).cursor(),
            value.len() - 1,
            "the cursor stays between the parens, unmoved"
        );
    })
    .unwrap();
    let _ = std::fs::remove_dir_all(&dir);
